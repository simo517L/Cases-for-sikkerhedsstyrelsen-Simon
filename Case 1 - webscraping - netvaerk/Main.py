...
def scrape_multi(start_url, website_depth = 2,max_order = 2):
	...
	website_dataframe = pd.DataFrame({'Website':start_url_domain, 'Order':0, 'Connected websites':[external_domain]})
	...
	while(i< max_order):
		...
		for url in external_urls:
			...
			if (url not in external_urls_checked) & (url_domain not in domain_checked):
				...
				dataframe_temp = pd.DataFrame({'Website': url_domain, 'Order': i, 'Connected websites': [external_domain_temp]})
		...
		if i == max_order:
			for url in external_urls:
				if url not in external_urls_checked:
					dataframe_temp = pd.DataFrame({'Website': url, 'Order': max_order+1, 'Connected websites': []})
					website_dataframe = website_dataframe._append(dataframe_temp)
